...
/*
 * Logos that only paint inside a document, so they must not be handed to the
 * rasteriser. See LOGO_NEEDS_DOM in src/lib/provider-directory.js — the provider
 * name is drawn instead, which is the same fallback a missing logo already gets.
 */
const NEEDS_DOM = new Set<string>(LOGO_NEEDS_DOM);
...
async function getLogos(siteUrl: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    /*
     * The file is `window.__PROVIDER_LOGOS = {…};`, generated by
     * scripts/build-provider-logos.mjs. Anchor on the assignment rather than the
     * first "{" in the file, so a future header comment cannot break the parse.
     */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const grouped = groupServicesByProvider(services);
  ...
  for (const g of displayProviders) {
    const svg = NEEDS_DOM.has(g.provider) ? undefined : logos[g.provider];
